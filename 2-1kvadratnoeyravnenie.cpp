...
int main(){
	...
	if (a == 0 || b == 0 || c == 0){
		if (a == 0 && b != 0 && c != 0){
			x = (-c) / b; // тут возможны проблемы при вычислениях
			cout << "корнем линейного уравнени¤ ¤вл¤етс¤ x=" << x;
		}
		if (a != 0 && b == 0 && c != 0){
			x = (-c) / a;// тут возможны проблемы при вычислениях
			if (x < 0){
		...
		if (a != 0 && b != 0 && c == 0){
			x = (-b) / a; // тут возможны проблемы при вычислениях
	...
	if (a == 0 && b == 0 && c != 0){
		...
		d = (pow(b,2) - (4 * a*c)); // а зачем функция pow нужна тут??????
	if (d > 0){
		x1 = ((-b) + sqrt(d)) / (2 * a);
		x2 = ((-b) - sqrt(d)) / (2 * a);
		cout << "корн¤ми квадратного уравнени¤ ¤вл¤ютс¤: x1="<<x1<<" и x2="<<x2;
	...
	if (d == 0){
		x0 = ((-b) / 2*a); // тут возможны проблемы при вычислениях
